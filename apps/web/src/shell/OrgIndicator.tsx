...
import { Building2, CheckIcon, ChevronDown } from '../icons';
 ...
 *
 *  - Single-org users: shows a compact read-only chip (icon + name + role).
 *  - Multi-org users: same chip becomes a button that opens a dropdown to
 *    switch the active org. The chosen org is what every other UI piece
 *    (workspace selector, admin console) scopes itself to.
 *
  ...
  return (
    <div ref={ref} className="relative" data-testid="org-indicator">
      <button
        ...
        disabled={!multi}
        className={`${chipBase} ${
          multi ? 'hover:border-brand/40 text-ink' : 'text-ink cursor-default opacity-90'
        }`}
      >
        ...
        {multi && <ChevronDown size={12} className="text-ink-muted" />}
      </button>
      {open && multi && (
        <div
          role="menu"
