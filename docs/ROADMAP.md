...
| No notion of **instance owner**, above organisation roles | 1-2 days | The embedding provider is instance-wide (ADR-003), so its admin routes have no organisation to scope by and the bar is `org_admin` of any org. On a single-org install — what Core targets — that IS the instance owner and there is no gap. On an installation shared by organisations that do not trust each other, one tenant's org_admin can change what the others search with. No data leaks and none is destroyed; closing it needs a role above the org, which the schema has not got. Documented and tested in `cross-org-isolation.integration.test.ts`. |
