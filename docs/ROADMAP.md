...
| ~~└ 48b: model switch with mass re-index~~ | 3-4 days | ✅ #113. Choosing a provider in Admin → AI does **not** flip the model: saving records it, and the flip is a separate, confirmed step that says what it costs. Each organisation gets its own partition and its own HNSW index, which is also why it is correct — a small tenant sharing an index with a large one recalled 0 of 5 of its own chunks, and 5 of 5 partitioned. Reindexing is still one synchronous pass. |
