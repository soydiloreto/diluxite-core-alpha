...
| └ 48c: more providers (Bedrock, OpenAI, Voyage, Cohere) | ~0.5 day each | All authenticate with a bearer API key in a header — **Bedrock included**, via [Bedrock API keys](https://docs.aws.amazon.com/bedrock/latest/userguide/api-keys-use.html); no AWS SDK and no SigV4. |
...
| No notion of **instance owner**, above organisation roles | 1-2 days | The embedding provider is instance-wide (ADR-003), so its admin routes have no organisation to scope by and the bar is `super_admin` of any org. On a single-org install — what Core targets — that IS the instance owner and there is no gap. On an installation shared by organisations that do not trust each other, one tenant's super_admin can change what the others search with. No data leaks and none is destroyed; closing it needs a role above the org, which the schema has not got. Documented and tested in `cross-org-isolation.integration.test.ts`. |
...
| **Occasional flakes when the two vitest projects share a database** | 1 day | Three seen, all passing in isolation and on re-run: audit cursor pagination, `last_login_at` on a trusted-header request, and note ordering by `updatedAt`. All look like same-timestamp ordering under concurrency rather than product bugs, but "looks like" is not "is". Worth a deliberate look: either separate databases per project, or ordering that does not depend on clock resolution. |
