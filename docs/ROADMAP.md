...
| | Effort | Notes |
|---|---|---|
| **1. Provenance, validity and rank** | 3-4 days | **No value is ever returned without the date it was true.** ADR-002 specifies the shape: three orthogonal axes, each from an existing standard — **PROV-O** (origin note+line, the agent it is attributed to, what it was derived from), **SQL:2011 bitemporal** (`valid_from`/`valid_to` for the world's timeline, `recorded_at` for ours), and **Wikidata ranks** (`preferred`/`normal`/`deprecated`, where superseded is kept rather than deleted). "MRR 42k (12 minutes ago)" is something you say out loud in a meeting; "MRR 42k (March)" is something you go check. Enabler for both rows below; improves what ships today on its own, and retrofitting it later is the expensive order. |
| **1b. Decay estimated from observed change** | 1-2 days | **No knowledge classes and nothing to declare.** Each item keeps a running estimate of how often it actually changes (EWMA over its own edit intervals, which `note_versions` has been recording since migration 0023). Maintained on save in constant time — **no scheduled job, no pass over the corpus**; staleness is a subtraction at query time over the results actually returned. Cold start uses a prior from STRUCTURE, not topic: measured on Wikipedia, a lead sentence has a 46-day median shelf life against 3,740 days for infobox fields. No model in this path — the answer to "why is this stale?" has to be a count, not a judgement. |
