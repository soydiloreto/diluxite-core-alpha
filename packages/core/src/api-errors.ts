...
export const API_ERRORS: Catalog = {
  ...
  'instance.requiresSetupAdmin': {
    en: 'this setting belongs to whoever runs the installation',
    es: 'esta configuración es de quien opera la instalación',
    pt: 'esta configuração pertence a quem opera a instalação',
    it: 'questa impostazione appartiene a chi gestisce l’installazione',
    ca: 'aquesta configuració és de qui opera la instal·lació',
    zh: '此设置属于运行该安装的人',
  },
  ...
  'import.zipRequired': {
    ...
    pt: 'envie o export como ZIP em base64 em body.zipBase64',
    it: 'invia l’export come ZIP in base64 in body.zipBase64',
    ca: 'envia l’export com a ZIP en base64 a body.zipBase64',
    zh: '请在 body.zipBase64 中以 base64 ZIP 发送导出文件',
  },
  'import.tooLarge': {
    en: 'that export is too large to import in one go (max 20MB)',
    es: 'ese export es demasiado grande para importar de una vez (máx 20MB)',
    pt: 'esse export é grande demais para importar de uma vez (máx 20MB)',
    it: 'questo export è troppo grande per importarlo in una volta (max 20MB)',
    ca: 'aquest export és massa gran per importar-lo de cop (màx 20MB)',
    zh: '该导出文件过大，无法一次性导入（上限 20MB）',
  },
  'import.notAZip': {
    en: 'that file could not be read as a ZIP',
    es: 'ese archivo no se pudo leer como ZIP',
    pt: 'esse arquivo não pôde ser lido como ZIP',
    it: 'quel file non è leggibile come ZIP',
    ca: 'aquest fitxer no s’ha pogut llegir com a ZIP',
    zh: '无法将该文件读取为 ZIP',
  },
  'note.tagManyIdsRequired': {
    en: 'select at least one note to tag',
    es: 'elegí al menos una nota para etiquetar',
    pt: 'selecione ao menos uma nota para etiquetar',
    it: 'seleziona almeno una nota da etichettare',
    ca: 'tria almenys una nota per etiquetar',
    zh: '请至少选择一条要添加标签的笔记',
  },
  'note.tagManyTagsInvalid': {
    en: 'not a usable tag: {tag}',
    es: 'no es un tag válido: {tag}',
    pt: 'não é uma etiqueta válida: {tag}',
    it: 'non è un tag valido: {tag}',
    ca: 'no és una etiqueta vàlida: {tag}',
    zh: '不是有效的标签：{tag}',
  },
  'note.tagManyNothingToDo': {
    en: 'say which tag to add or remove',
    es: 'decí qué tag agregar o quitar',
    pt: 'diga qual etiqueta adicionar ou remover',
    it: 'indica quale tag aggiungere o togliere',
    ca: 'digues quina etiqueta afegir o treure',
    zh: '请说明要添加或移除哪个标签',
  },
  'note.tagManyRefused': {
    en: 'none of those notes can be edited with this account',
    es: 'ninguna de esas notas se puede editar con esta cuenta',
    pt: 'nenhuma dessas notas pode ser editada com esta conta',
    it: 'nessuna di quelle note è modificabile con questo account',
    ca: 'cap d’aquestes notes es pot editar amb aquest compte',
    zh: '此账户无法编辑这些笔记中的任何一条',
  },
  'note.notInTrash': {
  ...
  'org.lastAdminRemove': {
    en: 'cannot remove the last org_admin',
    es: 'no se puede quitar al último org_admin',
    pt: 'não é possível remover o último org_admin',
    it: 'non si può rimuovere l’ultimo org_admin',
    ca: 'no es pot treure l’últim org_admin',
    zh: '无法移除最后一位 org_admin',
  },
  'org.lastAdminDemote': {
    en: 'cannot demote the last org_admin',
    es: 'no se puede degradar al último org_admin',
    pt: 'não é possível rebaixar o último org_admin',
    it: 'non si può declassare l’ultimo org_admin',
    ca: 'no es pot degradar l’últim org_admin',
    zh: '无法降级最后一位 org_admin',
  },
  'search.invalidMode': {
